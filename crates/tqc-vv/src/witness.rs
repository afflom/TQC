...
pub fn solovay_kitaev_probe(p: &UseCaseParams) -> Result<SolovayKitaevMetrics, String> {
    ...
    let is_z_cyclotomic = |z: f64| -> bool {
        ...
        // The cyclotomic field of the Atlas is generated by roots of unity of order bounded
        // by the generator orders (e.g., sigma_order = 96). Thus, any root of unity in this
        // algebraic extension has degree bounded well below 256.
        // A bound of n <= 256 ensures we are performing a finite exact algebraic check,
        // rather than a dense floating-point proximity search, since the closest rational
        // with n <= 256 to an irrational is bounded away by ~1/n^2 > 1.5e-5 >> 1e-7.
        for n in 1..=256 {
